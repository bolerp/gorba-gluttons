-- 🚨 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ функции достижений
-- Устраняет ошибку: column reference "achievement_id" is ambiguous
-- Запустите НЕМЕДЛЕННО в Supabase SQL Editor

DROP FUNCTION IF EXISTS check_and_unlock_achievements(TEXT);

-- 🔧 Полностью переписанная функция без амбiguous column
...
BEGIN
    ...
    IF player_rank <= 10 AND NOT EXISTS (
        SELECT 1 FROM player_achievements pa WHERE pa.wallet_address = p_wallet_address AND pa.achievement_id = 'top_10'
    ) THEN
    ...
    IF player_rank <= 3 AND NOT EXISTS (
        SELECT 1 FROM player_achievements pa WHERE pa.wallet_address = p_wallet_address AND pa.achievement_id = 'top_3'
    ) THEN
    ...
    IF player_rank = 1 AND NOT EXISTS (
        SELECT 1 FROM player_achievements pa WHERE pa.wallet_address = p_wallet_address AND pa.achievement_id = 'king_of_heap'
    ) THEN
    ...
    IF total_players <= 100 AND NOT EXISTS (
        SELECT 1 FROM player_achievements pa WHERE pa.wallet_address = p_wallet_address AND pa.achievement_id = 'early_bird'
    ) THEN
    ...
    -- 🎁 ВОЗВРАЩАЕМ новые достижения (БЕЗ АМБIGUOUS COLUMN!)
    RETURN QUERY
    SELECT ach.id, ach.name, ach.description, ach.icon
    FROM achievements ach
    WHERE ach.id = ANY(newly_unlocked);
END;
$$ LANGUAGE plpgsql;

-- ✅ ГОТОВО! Функция полностью исправлена. 